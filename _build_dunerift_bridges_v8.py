"""Dune Rift - Two Bridges v8: map-wide mesh sync - v7 synced (and VERIFIED) the render
mesh only inside the causeway rects, so 177 tiles of stale terracing divergence shipped.

User report (23 Jul, photo): structure near the east shore placed half INSIDE a drawn
dune. Diagnosis (diag scan of the installed v7 file): the 3v3 base carries ~177 tiles
where the render mesh sits up to +40 ABOVE the heightfield on dry land (leveled pans
under visual dunes, inherited by every derived map). Structures placed there bury and
units driving through vanish - the exact class v7 fixed, but only inside its rects;
v7's mesh gate also only asserted inside the rects, so the rest of the map was never
checked. Collision on the installed v7 file re-verified CLEAN (worst -0.29, 0 verts
at/above ground) - the turret-no-fire report is NOT stale collision on this map; it is
being investigated separately (deck-edge LOS shadow vs railing props).

Changes vs v7 (everything else byte-identical rebuild from the same base):
- render mesh: forced vert sync extended MAP-WIDE on dry cells - SINK-direction only
  (y > hf+0.15 pulled down to hf-0.15). Float-direction left alone outside rects on
  purpose: the draped sea mesh over hfield-dry strips (e.g. x512-543 z672-703, -20)
  would otherwise rise into new mid-lake "sandbars" and change the map's read; the
  bridges-only-land-route invariant must stay visually believable.
- verification: the mesh above-ground gate now scans ALL dry in-map verts (skip
  underplane y<2, water-sheet band |y-WL|<0.8, wet cells), not just the rects.
"""
...
# ---- terrain: capped deck planes + aprons (identical to v7) ----
...
# ---- render mesh: delta resample + FORCED sync (v8: map-wide on dry land) ----
...
for i in range(nv):
    ...
    if abs(y - WL) < 0.8:                           # water sheet: leave
        continue
    if H[int(z)*w + int(x)] <= WR:                  # wet cell: lake bed / sheet shells
        continue
    gy = sm.hf_sample(H, w, x, z) - 0.15            # surface record hugs ground
    if in_rect(x, z):
        if abs(y - gy) > 0.3:                       # v7-proven behavior in rects
            struct.pack_into("<3f", pb, off, x, gy, z)
            forced += 1
    elif y - gy > 0.3:                              # map-wide: sink-direction only
        struct.pack_into("<3f", pb, off, x, gy, z)
        lowered += 1
terr_new = sm.rebuild_bdf(terr_new, bytes(pb))
print(f"mesh: {mv} delta-tracked, {forced} force-synced in rects, "
      f"{lowered} sink-synced map-wide", flush=True)

# ---- collision: global snap to ground (identical to v7; idempotent) ----
...
# ---- nav: legacy overlay (identical to v7) ----
...
# ---- verification (nav gates identical to v7) ----
...
# mesh/hfield sync check - v8: MAP-WIDE on dry land (the gate v7 lacked)
...
print(f"mesh worst ABOVE ground map-wide (dry): {worst:.2f} at {wxz}", flush=True)
assert worst < 0.3
# collision check
...
# ---- props: railings from band props (identical to v7) ----
...
# ---- waterDepth: retarget + regenerate (identical to v7) ----
...
# ---- minimap: paint the causeways (identical to v7) ----
