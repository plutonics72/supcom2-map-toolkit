...
for tag, cells in islands.items():
    ...
    print(f"island {tag}: +{len(added)} cells (target {target})", flush=True)
...
for tag, cells in grown.items():
    ...
    cand = [j for j in cells if dist_wet[j] >= 14]
    placed = 0
...
for i in range(G*G):
    ...
    for li in waterish:
        o = layers[li][2]
        v = 255 if dn and not ds else stock[o + i]
        if pay[o + i] != v:
...
shutil.copy2(MAP, MAP + ".V21.bak")
out = dict(ent)
out[[n for n in names if n.endswith(".hfield.win.bdf")][0]] = hf_new
out[[n for n in names if n.endswith(".terrain.win.bdf")][0]] = terr_new
out[[n for n in names if n.endswith(".costs.win.bdf")][0]] = costs_new
out[[n for n in names if n.endswith("_save.lua")][0]] = sav.encode("utf-8")
