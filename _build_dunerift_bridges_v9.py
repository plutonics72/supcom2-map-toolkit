"""Dune Rift - Two Bridges v9: nav noise repair - open flat dry ground, seal the ghost ford.

User report (24 Jul, three videos): units ordered west -> central bridge stall "at
various points": extreme single-file conga lines across open desert, a pileup at the
west-approach palm grove, one unit stopping dead at a "causeway stub that dips under
water" SW of the real bridge, then bouncing/sliding along invisible edges.

Diagnosis (measured on the installed v8 file):
- Decks are HEALTHY: mesh at deck height (byte-equal v7<->v8), costs 1 / island 0 on
  all five layers across both spans. Not the bridge.
- The west approach & central basin dry ground is riddled with nav-255 patches on
  flat walkable-looking cells (40-100 blocked cells per 16x16 tile around x352-447
  z592-655), inherited from the base 3v3 bake (pre-terracing flood noise) plus the
  old-ford closure. Units thread the gaps single-file (conga), jam at gap mouths,
  and stop at patch edges mid-desert.
- The ORIGINAL 3v3 ford ((360,701)->(470,561) diagonal) still exists as raised dry
  hfield with open ramps; its water segment is blocked -> an open dead-end spit that
  lures units to a dead stop at the water line (the "submerged stub" in vid3).

Fix vs v8 (decks/mesh/collision/waterDepth/props/minimap byte-identical rebuild),
