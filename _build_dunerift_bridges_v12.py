"""Dune Rift - Two Bridges v12 (lobby v9): the "dried oasis" honesty fix.

User report (25 Jul, insisted against my auto-engage theory - correctly): right-
clicking the pale lakebed area NW of the central plateau sends the commander on a
specific path EAST instead. Measured: that gully system (two arms + pond, NW bowl)
renders as pale DRY bed (mesh ~25, water surface not drawn there) but its hfield is
17-26 - far below waterline 34 - so land-nav is closed and the engine silently
reroutes any order clicked there to its deterministic nearest-reachable. A visual
lie inherited from stock (campaign "dried oasis" art over below-waterline terrain).

v12 raises the gully bed to WL+1.5 (real dry land; the delta resample raises the
drawn bed to match, overlay opens land nav via wet_b, waterDepth dry-marks follow)
and grades two cut-only entry ramps so the basin connects instead of becoming a
reverted pocket. New gates: oasis bed cells open + island 0 + r=3 reachable from
the west spawn.

Carried over from v11 (lobby v8): wet-aware hover walls (v10's blanket fence had
sealed the southern channel), shore-ribbon + hover-land unify + stock water-layer
semantics (campaign bake left hover holes on slopes and closed patches of open
water), hard gates on water-body island unity.
...
NAME = "[6] Dune Rift - Two Bridges (3v3) v9"   # version IN the lobby name = mount proof; bump EVERY build
OASIS_H = WL + 1.5                              # dried-oasis bed target (real dry land)
OASIS = [(356, 296, 476, 420)]   # NW gully + pond ONLY - z>420 is the rift lake's
                                 # NW bay (a second rect there drained the lake and
                                 # broke the bridges-only invariant; gate caught it)
...
# ---- dried-oasis honesty: raise below-waterline "dry-looking" gully beds ----
raised = 0
for (ox0, oz0, ox1, oz1) in OASIS:
    for z in range(oz0, oz1 + 1):
        for x in range(ox0, ox1 + 1):
            if H[z*w + x] <= WR + 256:      # include the old-waterline hair ring (<=36.0)
                setH(x, z, OASIS_H); raised += 1
print(f"oasis: {raised} cells leveled to {OASIS_H}", flush=True)
...
r1 = cut_ramp(452, 322, 472, 346, "x", 472, 44.0, 452, OASIS_H + 0.3)  # terrace -> north arm
r2 = cut_ramp(408, 352, 444, 378, "z", 378, 44.0, 352, OASIS_H + 0.3)  # isthmus -> gully tip
print(f"oasis ramps: {r1} + {r2} cells graded", flush=True)
...
for z in range(G):
    ...
    for x in range(G):
        ...
        if wet_now:
            ...
            for li in wateronly:      # stock semantics (FORMATS.md): water is navigable
                if pay[layers[li][2] + i] == 255:   # for hover/amphib/naval by design;
                    pay[layers[li][2] + i] = 1      # the campaign bake left closed patches
...
# hover-land unification (stock semantics per README: all five layers are open on
# ordinary walkable land). The campaign bake never baked hover outside its mission
# area, leaving hover holes across walkable slopes and patchy hover water - these
# holes disconnect shore-adjacent terraces from the main hover island.
unified = 0
...
for i in range(G*G):
    if pay[oL + i] != 255:
        ...
        for li in wateronly:
            if pay[layers[li][2] + i] == 255:
                pay[layers[li][2] + i] = 1; ch = True
        if ch: unified += 1
print(f"hover-land unify: {unified} cells opened", flush=True)
...
# shore-ribbon hover repair: the bake leaves a 1-cell closed ribbon along shorelines
# whose lip exceeds the 1.2 gentle() threshold (measured 1.6-2.0 on the channel's
# east shore) - it seals whole water bodies off from hover/amphib traffic even
# though land and water are both open. Open the ribbon on HOVER layers only, where
# the step down into the water is a beach lip (<=2.5/cell), never a cliff.
RIBBON_STEP = int(2.5 * 128)
RIBBON_DEPTH = 6
# near-water band: multi-source BFS from every wet cell, depth<=RIBBON_DEPTH
near_shore = bytearray(G*G)
seenb = bytearray(G*G)
qq = deque()
for z in range(G):
    b25 = z*w
...
while qq:
    ...
    for dx, dz in ((1, 0), (-1, 0), (0, 1), (0, -1)):
        ...
        if 0 <= nx < G and 0 <= nz < G and not seenb[nz*G + nx]:
            seenb[nz*G + nx] = 1
            near_shore[nz*G + nx] = 1
            qq.append((nx, nz, d + 1))
band = [(i % G, i // G) for i in range(G*G) if near_shore[i]]
# iterative opening: hover openness spreads across steps <=2.5 inside the band
# (multi-step beach lips connect; cliffs >2.5/step still seal)
oh0 = layers[wateronly[0]][2]
opened_total = 0
for _pass in range(RIBBON_DEPTH):
    opened = 0
    for (x, z) in band:
        i = z*G + x
        if pay[oh0 + i] != 255:
        ...
        for dx, dz in ((1, 0), (-1, 0), (0, 1), (0, -1)):
            ...
            if not (0 <= nx < G and 0 <= nz < G):
                continue
            if abs(h0 - H[nz*w + nx]) > RIBBON_STEP:
                continue
    ...
    if not opened:
        break
print(f"shore ribbon: {opened_total} hover cells opened in band", flush=True)

# hover edge walls, v11 WET-AWARE: v10 closed the whole edge row and disconnected
# the southern channel's hover island (red no-entry on all water orders there).
# The boarding move is stepping from WET water onto the DRY deck face - so close
# only DRY edge-row cells within Chebyshev-2 of a wet cell; open water stays open
# and the water bodies stay connected to the main hover island.
def near_wet(x, z):
    for dz in range(-2, 3):
        for dx in range(-2, 3):
            nx, nz = x + dx, z + dz
            if 0 <= nx < G and 0 <= nz < G and H[nz*w + nx] <= WR:
                return True
    return False
...
for (rx0, rz0, rx1, rz1) in RECTS:
    for z in list(range(rz0, rz0 + EDGE)) + list(range(rz1 - EDGE + 1, rz1 + 1)):
        for x in range(rx0, rx1 + 1):
            if H[z*w + x] <= WR or not near_wet(x, z):
                continue                      # wet cells + inland cells stay open
            i = z*G + x
            for li in wateronly:
                if pay[layers[li][2] + i] != 255:
                    pay[layers[li][2] + i] = 255; edge_closed += 1
print(f"hover edge walls (wet-aware): {edge_closed} layer-cells closed", flush=True)
...
for (x0, z0, x1, z1, tg) in [(430, 552, 692, 620, "A"), (438, 754, 680, 823, "B")]:
    ...
    for li in range(5):
        ...
        allok &= n_closed == 0
    for li in wateronly:   # near-wet DRY edge cells must stay hover-closed (no boarding)
        o5 = layers[li][2]
        n_bad = sum(1 for zz in (z0, z1) for x in range(x0, x1+1)
                    if H[zz*w + x] > WR and near_wet(x, zz) and pay[o5 + zz*G + x] != 255)
        print(f"deck {tg} hover layer {li}: {n_bad} unwalled near-wet edge cells (want 0)", flush=True)
        allok &= n_bad == 0

# v11 gate: water bodies must share the MAIN hover island (no red-cursor water
# pockets; regression guard for the v10 fence bug). The south canyon is SOFT:
# its sheer 40+ walls may legitimately seal it (design), so report, don't fail.
for li in [wateronly[0], wateronly[1]]:
    offB5 = layers[li][4]
    main_isl = pay[offB5 + 600*G + 300]        # main land reference
    for wn, wx, wz, hard in [("north lake", 480, 480, True), ("channel", 600, 690, True),
                             ("channel2", 590, 720, True), ("south canyon", 560, 840, False)]:
        isl = pay[offB5 + wz*G + wx]
        ok = isl == main_isl
        verdict = "OK" if ok else ("FAIL" if hard else "sealed (canyon; acceptable)")
        print(f"hover L{li} {wn}: island {isl} (main {main_isl}) {verdict}", flush=True)
        if hard: allok &= ok

if not allok:
    # RIM AUTOPSY: what encloses the channel's hover component?
    ohc = layers[wateronly[0]][2]
    mch = bytearray(1 if pay[ohc + i] != 255 else 0 for i in range(G*G))
    comp = flood(mch, snapc(mch, 600, 690))
    ...
    for z in range(1, G-1):
        for x in range(1, G-1):
            ...
            if pay[ohc + i] != 255:
                continue
            if not any(comp[(z+dz)*G + x+dx] for dx, dz in ((1,0),(-1,0),(0,1),(0,-1))):
                continue
            key = ("wet" if H[z*w+x] <= WR else "dry",
                   "rect" if in_rect(x, z) else "open",
                   "band" if near_shore[i] else "inland")
            reasons[key] += 1
            if len(samples) < 25:
                samples.append((x, z, round(H[z*w+x]/128.0, 1), key))
    print(f"RIM AUTOPSY: {dict(reasons)}", flush=True)
    for s in samples:
        print(f"   rim {s}", flush=True)

# v12 gate: the oasis bed must be real, open, main-island, reachable land now
oisl = layers[land_layers[0]][4]
for nm2, px, pz in [("oasis N arm", 416, 330), ("oasis pond", 424, 356), ("oasis W bed", 390, 310)]:
    open_ok = pay[o + pz*G + px] != 255
    isl_ok = pay[oisl + pz*G + px] == 0
    print(f"{nm2} ({px},{pz}): open={open_ok} island0={isl_ok} h={H[pz*w+px]/128.0:.1f}", flush=True)
    allok &= open_ok and isl_ok
ok_oasis = reach(erode(m0, 3), (150, 500), (416, 330))
print(f"[r=3] west spawn -> oasis bed: {'OK' if ok_oasis else 'FAIL'}", flush=True)
allok &= ok_oasis
