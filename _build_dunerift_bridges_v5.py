"""Dune Rift - Two Bridges v5: v4 + deck floor WL+8 + waterDepth regen/retarget.
    ...
    for x in range(G):
        ...
        if wet_now:
            ...
            if wet_b:
                ...
                walled += 1
print(f"nav: closed {closed}, opened {opened}, naval walls {walled}; islands...", flush=True)
    ...
    for bx in range(ww // 4):
        ...
        for cz in range(cz0, min(G, cz0 + 4*scale)):
            for cx in range(cx0, min(G, cx0 + 4*scale)):
                if H[cz*w + cx] <= WR: all_dry = False; break
