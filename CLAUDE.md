...
| [6] Boolon Isles (3v3) v11 | `_boolon_isles.scd` | `SC2_BOOLI1` | installed on the LAPTOP (10 Aug), in-game verified BY CLAUDE — NOT on Drive. Boolon's TRUE topology as real terrain islands; v2-v3 layout spec, v4-v7 render-mesh war (crater rims, sea-stack spires, sunken shoreline rings/ponds — see git log). v8-v11 (user photo, 10 Aug): (1) MASS POINTS root cause = pad pass and lua pass used different relocation rules — 15/36 markers shipped stranded at the shoreline off their leveled pads while the gate validated the pad list; markers now snapped to pads + gate validates the SHIPPED save.lua (extractors built in-game on previously-broken markers, 2500/2500); (2) ring debris: 3,684 flushed crater-rock tris deleted from the index buffer (greedy floor-cover, strict per-cell coverage — map-wide scope and 1-cell dilation both A/B-proven to open coast holes, reverted), kept floor tris re-attributed to a per-record slab (cross-record attr copies garble); (3) minimap/lobby previews per-pixel rendered from final heights (solid-block DXT1). Awaiting user playtest |
